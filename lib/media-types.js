...
export const parseResponse = (response) => {
  ...
  if (contentTypeText === null) {
    throw new UnknownMediaTypeError("The media type of the response could not be determined. Make sure the response inclues a 'Content-Type' header.", { cause: response });
  }

  const contentType = parseContentType(contentTypeText);
  if (!(contentType.type in mediaTypePlugins)) {
    throw new UnsupportedMediaTypeError(`'${contentType.type}' is not supported. Use the 'addMediaTypePlugin' function to add support for this media type.`, {
      cause: response
    });
  }
