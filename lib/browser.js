...
export const get = async (uri, browser = undefined) => {
  if (browser) {
    uri = resolveIri(uri, browser.uri);
  }
  ...
  if (responseBrowser._value instanceof Reference) {
    const referenceUri = resolveIri(responseBrowser._value.href, response.url);
